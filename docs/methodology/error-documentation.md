...
- **Root Cause**: Two independent facts, neither of them an application bug.
  1. The destination was `jane@company.com`. **`company.com` publishes no MX record at all** (`dig +short MX company.com` → empty), so SMTP falls back to its A/AAAA records, which are Cloudflare proxy IPs serving HTTP only. Verified from the production host: port 25 → *Connection refused*, port 443 → open. `company.com` is a third-party domain on Cloudflare nameservers — **nothing on our side can fix it**, and `xpandia.global` itself is healthy (`MX → aspmx.l.google.com`, SPF and DMARC present).
  2. `jane@company.com` was **our own contact-form email placeholder** (`frontend/messages/en/contact.json`), also filled as a literal value by `fillContactForm` in `frontend/e2e/fixtures.ts`. A real, live domain was being presented to users as an example address.
  The application was **not** the sender in this incident: nginx access logs covering 2026-07-22 → 2026-08-05 record zero `POST /api/contact/`, and neither `backend/logs/django.log` nor the `xpandia-huey` journal shows contact activity. The message was sent by hand from the Workspace mailbox. There is no auto-forwarder to pause.
- **Second send vector, closed by the same fix**: `e2e/public/interactions.spec.ts` has three tests using `fillContactForm`; two stub the API with `page.route`, but the happy-path test (`submitting the contact form shows the success banner`) does **not** — it waits for a real `201` from `/api/contact/`. Playwright's `webServer` runs `manage.py runserver`, which defaults to `settings_dev` (`manage.py:9`), and `settings_dev.py:27` selects the SMTP backend whenever credentials exist — which they do in `backend/.env` on the production host. Running that spec here would therefore have sent a real auto-reply from `milena@` to a stranger's domain. **Note that this path bypasses nginx** (it hits `127.0.0.1:8001` directly), so the nginx-log evidence above does not by itself rule out an E2E run. It was ruled out separately: the last Playwright run was 2026-08-01 16:37 UTC and `frontend/.next/dev/logs/next-development.log` shows it only ever compiled `/[locale]/services/language-assurance` — the contact page was never loaded — and that timestamp falls outside Gmail's ~24 h retry window for a send on 2026-08-03.
- **Resolution**: Moved the email placeholders (en/es) and the E2E fixture to `example.com`, reserved for documentation by RFC 2606, so an example address can never be a real mail destination. Separately, `EmailService` no longer swallows send exceptions and the view logs a warning when the auto-reply fails — previously a failed confirmation left no trace whatsoever.
- **Known limitation**: this only covers **synchronous** SMTP failures. Gmail bounces asynchronously (~24 h of retries) to the authenticated sender's mailbox, which the application never reads. Detecting undeliverable recipients automatically would require inbound bounce processing, which does not exist. Operationally, the human who owns the sending mailbox must forward bounces.
- **Diagnostic recipe** for a repeat report: `dig +short MX <domain>` (empty ⇒ SMTP falls back to A/AAAA), then `dig +short A <domain>` and check whether the IPs belong to a web proxy (`whois <ip> | grep NetName`), then confirm with a port-25 probe. Before blaming DNS, grep the nginx logs for `POST /api/contact/` to establish whether the application sent anything at all — and remember that a local E2E run bypasses nginx, so also check the Playwright artifact timestamps (`frontend/test-results/.last-run.json`, `frontend/e2e-results/results.json`) and the Next dev log.
- **Files Affected**: `frontend/messages/en/contact.json`, `frontend/messages/es/contact.json`, `frontend/e2e/fixtures.ts`, `backend/base_feature_app/services/email_service.py`, `backend/base_feature_app/views/contact.py`
