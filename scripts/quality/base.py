...
class Config:
    ...
    # F46 (2026-08-17): sharded flow registry. Empty string (default) = the
    # classic monolith at <e2e>/flow-definitions.json. A dir name (relative to
    # the e2e root, e.g. "flows") = one JSON file per flow inside it; the
    # aggregate views (flow-tags.js, USER_FLOW_MAP.md) become DERIVED artifacts
    # regenerated by generate_flow_registry.py, so parallel PRs stop colliding
    # on a single registry file. Both layouts are first-class: the core is
    # propagated ahead of per-project migration and must keep working on both.
    flow_definitions_dir: str = ""
