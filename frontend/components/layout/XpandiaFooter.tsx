...
export default async function XpandiaFooter() {
  const t = await getTranslations('common.footer')
  const s = await getTranslations('common.serviceLines')
    ...
    <footer className="pt-20 pb-10 bg-ink-900 text-ink-300">
      <div className="w-full max-w-[1280px] mx-auto px-[clamp(24px,4vw,64px)]">
        <div className="grid grid-cols-1 sm:grid-cols-2 tablet:grid-cols-[1.7fr_1fr_1fr_1fr] gap-8 sm:gap-10 tablet:gap-12 pb-14 border-b border-white/[0.08]">
          ...
          <div>
            <h5 className="font-mono text-[11px] tracking-[0.12em] uppercase text-ink-400 mt-0 mb-4 font-medium">{t('colServices.heading')}</h5>
            <ul className="list-none p-0 m-0">
              <li className="mb-[10px] text-[14px]"><Link href="/services/language-assurance" className="text-ink-200 hover:text-white transition-colors">{s('languageAssurance')}</Link></li>
              <li className="mb-[10px] text-[14px]"><Link href="/services/localization-adaptation" className="text-ink-200 hover:text-white transition-colors">{s('localizationAdaptation')}</Link></li>
              <li className="mb-[10px] text-[14px]"><Link href="/services/applied-cultural-intelligence" className="text-ink-200 hover:text-white transition-colors">{s('appliedCulturalIntelligence')}</Link></li>
            </ul>
          </div>
          <div>
            <h5 className="font-mono text-[11px] tracking-[0.12em] uppercase text-ink-400 mt-0 mb-4 font-medium">{t('colCompany.heading')}</h5>
            <ul className="list-none p-0 m-0">
