...
description: "Audit bidirectional sync entre este ops repo (workflows/) y (a) per-project skills (.claude/skills, .agents/skills, .windsurf/workflows) y (b) user-level config ($HOME/.claude, $HOME/.codex). Reporta gaps en ambas direcciones; --apply-* arguments para arreglar."
