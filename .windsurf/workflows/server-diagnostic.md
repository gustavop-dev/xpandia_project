...
- **Scripts de monitoreo**: `/home/ryzepeck/scripts/`
- **Reportes generados**: `/home/ryzepeck/scripts/reports/`
...
- Los proyectos bajo `inactive:` se **excluyen** de las evaluaciones operativas (gunicorn, huey, health checks, backups recientes). Solo se verifica que sus archivos/configs existan y estén conservados.
- **candle_project** y **kore_project** son los proyectos de referencia/template para evaluar los demás.
- **tenndalux_project** no tiene base de datos — no penalizar por falta de backups DB ni servicios inactivos.
...
# Extraer nombres de projects.yml (active + inactive)
KNOWN=$(grep '^\s*- name:' ~/webapps/projects.yml | awk '{print $3}')
KNOWN="$KNOWN ops"  # ops es infraestructura, no un proyecto
...
```
Si se detecta un proyecto nuevo, incluirlo en el diagnóstico y verificar que tenga **todos** los componentes estándar. Sugerir agregarlo a `projects.yml`.
